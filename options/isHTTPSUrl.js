...
export default (optionName, optionValue, optionValueType, path, type, value, inArray) => {
    ...
    try {
        ...
        if (parsedUrl.protocol === `https:` && parsedUrl.hostname.length) isUrl = true
        if (isUrl !== optionValue) {
            result = `${inArray ? `Array element in field` : `Field`} '${path}' must be${!optionValue ? ` not` : ``} https url.`
        }
        return result
    } catch (error) {
        if (isUrl !== optionValue) {
            result = `${inArray ? `Array element in field` : `Field`} '${path}' must be${!optionValue ? ` not` : ``} https url. '${value.toString()}' given.`
        }
        return result
    }
}