...
export default (optionName, optionValue, optionValueType, path, type, value, inArray) => {
    ...
    try {
        ...
        if (isUrl !== optionValue) {
            result = `${inArray ? `Array element in field` : `Field`} '${path}' must be${!optionValue ? ` not` : ``} http url.`
        }
        return result
    } catch (error) {
        if (isUrl !== optionValue) {
            result = `Field '${path}' must be${!optionValue ? ` not` : ``} http url. '${value.toString()}' given.`
        }
        return result
    }
}