        ...
        switch (fieldValueType) {
            case `Number`:
                if (fieldOptionValueType !== `Number`) {
                    result = `Field '${fieldPath}', 'isNumber' contains incorrect option 'min'. Value of 'min' must be type of 'Number'. '${fieldOptionValueType}' given.`
                }
                if (fieldValue < fieldOptionValue) {
                    result = `Field '${fieldPath}' must be less than '${fieldOptionValue.toString()}'. '${fieldValue.toString()}' given.`
                }
                break
            case `BigInt`:
                if (fieldOptionValueType !== `String`) {
                    result = `Field '${fieldPath}', 'isBigInt' contains incorrect option 'min'. Value of 'min' must be type of 'String' because this is 'BigInt'. '${fieldOptionValueType}' given.`
                }
                ...
                break
            default:
                result = `Field '${fieldPath}' contains incorrect option 'min'.`
                break
        }
