...
export default class {
    ...
    /**
     * Checks if a field value is a given option value and returns an error message if not.
    ...
    static fn = (fieldPath, fieldValue, fieldValueType, fieldOptionValue, fieldOptionValueType) => {
        ...
        if (net.isIPv4(fieldValue) !== fieldOptionValue) {
            result = `Field '${fieldPath}' must be${!fieldOptionValue ? ` not` : ``} IPv4. '${fieldValue.toString()}' given.`
        }
        return result
    }
}