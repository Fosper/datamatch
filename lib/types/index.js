...
export default class {
    static availableTypeOptions = {
        ...
        isNull: [],
        isBoolean: [],
        isNumber: [ `isFloat`, `isInt`, `isNumeric`, `max`, `maxLength`, `min`, `minLength`, `values` ],
        isBigInt: [ `isInt`, `isNumeric`, `max`, `maxLength`, `min`, `minLength`, `values` ],
        isString: [ `isDomain`, `isFloat`, `isHTTPSUrl`, `isHTTPUrl`, `isInt`, `isIP`, `isIPv4`, `isIPv6`, `isNumeric`, `isUrl`, `isWSSUrl`, `isWSUrl`, `maxLength`, `minLength`, `values` ],
    ...
    static default = (requiredFieldValueType, opt) => {
        ...
        let fieldPath = Object.prototype.toString.call(opt[2]) === `[object String]` ? opt[3] : ``
        ...
        (requiredFieldValueType, fieldPath, fieldValue, fieldValueType) => {
            ...
            if (requiredFieldValueType !== fieldValueType) {
                result = `Field '${fieldPath}' must be '${requiredFieldValueType}'. '${fieldValueType}' given.`
            }
            return result
        })
    }
